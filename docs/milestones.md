...
> **複数ツールの Swoole 並列実行**（Swoole ランタイム前提・`ParallelStreamingAgent`）は D29 / [`tasks-parallel-tools.md`](tasks-parallel-tools.md)（スパイク S-c 検証済み）
...
- `TOOL_CALL_ARGS` 引数ストリーミング（ToolUse feedback 反映待ち）
  - ※ **複数ツールの `toolCallId` 対応づけ**は M3 の並列ツール実行（D29 / [`tasks-parallel-tools.md`](tasks-parallel-tools.md)）で
