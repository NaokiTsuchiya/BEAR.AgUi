...
interface ToolCallView {                            // 読み手＝AgUiAdapter
    public function takeStarted(string $toolName): ?StartedToolCall; // 名前ごとの FIFO で {id, name}
...
```
                       ...
                       │  recordStart / appendInput      recordResult        takeStarted /  │
                       │        ▲(S5)                        ▲(S5)            resultFor (S2)  │
                       │        │                            │                     │         │
...
- **タイムライン**（いつ何が起きたか）は `AgentEvent` が運ぶ。**不足データ**（実 id/引数/結果 content）は
  `ToolCallRegistry` が横から供給。Adapter はこの 2 つを突き合わせる（`tool_start` で名前ごとの FIFO から
  id を取り、`tool_result` はその id でペア。並列 dispatch（D29）で異名ツールの結果順が乱れても
  id 対応づけは崩れない、D9 改訂/D10）。
