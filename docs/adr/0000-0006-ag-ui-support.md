...
### Context
AG-UI は `run(input: RunAgentInput) -> Observable<BaseEvent>` という関数抽象を、HTTP POST + SSE（または WebSocket）で実体化したプロトコルである。約16種のイベント型（`RUN_STARTED` / `TEXT_MESSAGE_*` / `TOOL_CALL_*` / `STATE_SNAPSHOT` / `STATE_DELTA` / `RUN_FINISHED` / `RUN_ERROR` 他）を SSE で `data: {json}\n\n` 形式で送出する。
...
## ADR 0005: トランスポートとデプロイ（SSE / WebSocket、Swoole、AgentCore コントラクト）
...
> `/ping` ヘルスチェックも実装・確認済み。INTERRUPT は SSE + send() 往復で実装でき、WebSocket は
> 第二段階のまま据え置く（プロトタイプは SSE 経路で human-in-the-loop を実証済み）。

### Context
AgentCore の AG-UI コントラクトは以下を要求する:

- **Container**: Host `0.0.0.0` / Port `8080` / **ARM64**
- **Path**: `POST /invocations`（SSE ストリーム）、`GET /ping`（ヘルスチェック、`{"status":"Healthy", ...}`）、`/ws`（WebSocket、双方向）
...
AG-UI 仕様自体はトランスポート非依存（SSE デフォルト、WebSocket / webhook も許容）。
...
- **WebSocket (`/ws`) は第二段階**。双方向（ユーザー割り込み、`INTERRUPT` 系）が必要になった時点で追加。SSE 実装の Renderer は再利用できる（フレーム化のみ差分）。
...
- WebSocket を後回しにするため、`INTERRUPT` を前提とする対話的 UX は初期スコープ外になる（明示的な非対応範囲）。
...
- WebSocket `/ws` における `INTERRUPT` / 双方向イベントの設計（ADR 0005 第二段階）
