...
`StreamingAgentFactory` / `MessageHistoryMapper` / `tests/Fake`）完了。M1 の最終 API は [`decisions.md`](decisions.md) D23
（runner は `stream()` で生成のみ・host が `SseResponder`+sink で枠付け・parser は `ParseError` 返し）を正とする。
...
- [ ] `Bootstrap::buildRunner(): AgUiRunner`
  ...
  - [ ] `OpenAiStreamingLlmClient`（上記 client + mappers）＋ `DemoDispatcher` ＋ `[GetTimeTool, AskConfirmationTool]` ＋ systemPrompt で `StreamingAgentFactory` を構築
  - [ ] `AgUiRunner` を `factory + MessageHistoryMapper + new AgUiAdapter(NullLogger) + []`（inputProcessors）で組む（D23：encoder/logger は runner ではなく responder/adapter が持つ）
  - [ ] SSE 化用に `SseResponder(new SseEncoder())` も用意（host が `respond($runner->stream($input), $sink)` で枠付け）。`Bootstrap` は runner と responder の両方を公開
...
- [ ] pre-flight（接続レベル・**`respond()` 前**）：`RunAgentInputParser::parse($body)` が `ParseError` を返したら → **400** `{code:VALIDATION_ERROR, message}`（空 user content・不正 JSON も parse 段階で `ParseError`）
- [ ] parse 済み `$input` を `$responder->respond($runner->stream($input), new PhpSapiSseSink())` で配信（D23：`stream()` は遅延生成、`respond()` が SSE 化）
