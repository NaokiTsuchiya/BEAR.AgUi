...
`weather_get` / `news_get` / `reminder_put` / `package_search` の 4 つだけです。`message_post` は
...
**実践ツールのデモ** — `package_search` は他の3ツールと違い canned data ではなく、
...
`weather_get` 等と並列実行される様子も見せられます:
...
src/Resource/App/*.php          #[Tool] 付きツールリソース 5 本(package_search のみ実HTTP)
