...
4. **Swoole 並列ツール実行** — confirm 不要のツールが 1 ターンに複数呼ばれると
   ...
   `rot13_get` + `word_similarity_get` が1ターンで同時に呼ばれ、WaitGroup 経由で dispatch される
   (どちらも素の PHP 関数呼び出しで人工レイテンシは無いため、壁時計での逐次/並列の体感差は無い)
...
| `package_search` | `app://self/package` (GET) | `safe` | ✓ | Packagist への実HTTP呼び出し(canned値ではない実データ) |
| `rot13_get` | `app://self/rot13` (GET) | `safe` | ✓ | 並列対象①(PHP の `str_rot13()`) |
| `word_similarity_get` | `app://self/similarity` (GET) | `safe` | ✓ | 並列対象②(PHP の `similar_text()`/`levenshtein()`) |
| `sun_info_get` | `app://self/sun-info` (GET) | `safe` | ✓ | PHP の `date_sun_info()` で日の出/日の入り/南中を計算 |
...
**並列 run** — `rot13_get` + `word_similarity_get` が 1 ターンで呼ばれ、WaitGroup で並列 dispatch:

```console
curl -N -X POST http://127.0.0.1:8080/invocations \
    -H 'Content-Type: application/json' \
    -d '{"threadId":"t-1","runId":"r-1","messages":[{"id":"m-1","role":"user","content":"ROT13でBEAR Sundayを変換して、PHPとPHP8の類似度も教えて"}]}'
```

両ツールの `TOOL_CALL_RESULT` が `RUN_FINISHED{success}` までに届きます。どちらも人工レイテンシの
無い素の PHP 関数呼び出しなので、逐次実行との壁時計差は体感できません(D29 が示すのは
「1 ターンに複数呼ばれた plain ツールは WaitGroup で並列 dispatch される」という仕組みそのものです)。
...
`reminder_put` / `package_search` / `rot13_get` / `word_similarity_get` / `sun_info_get` の 5 つだけです。
`message_post` は `safeAndIdempotent` ポリシーが常に締め出します(実装とテスト:
[`tests/Integration/ExampleBear/InvocationsTest.php`](../../tests/Integration/ExampleBear/InvocationsTest.php))。

**実践ツールのデモ** — `package_search` は他のツールと違い canned data ではなく、
Packagist(`packagist.org`)の検索APIを実際に叩きます(`GuzzleHttp\Client` を
`ClientInterface` として DI 束縛、テストでは fake に差し替え・D22 と同じパターン)。
「Composer の bear/tool-use というパッケージを調べて」のような日本語プロンプトで、
他のツールと並列実行される様子も見せられます:

```console
curl -N -X POST http://127.0.0.1:8080/invocations \
    -H 'Content-Type: application/json' \
    -d '{"threadId":"t-1","runId":"r-3","messages":[{"id":"m-1","role":"user","content":"Composer の bear/tool-use というパッケージを調べて"}]}'
```
...
デモ入力は curl の 2 シナリオと同じです("ROT13でBEAR Sundayを変換して、PHPとPHP8の類似度も教えて" が並列 run、
...
src/Resource/App/*.php          #[Tool] 付きツールリソース 6 本(package_search のみ実HTTP)
